...
Este repositório centraliza todo o processo de geração dos Relatórios Executivos Mensais do projeto Solve for Tomorrow Latam (SFTL).

O objetivo é transformar diversos relatórios técnicos exportados do Google Analytics 4, Google Search Console e SEMrush em um único relatório executivo de até duas laudas, direcionado à tomada de decisão.
...
prompts/                 -> Prompts e metodologia de análise
history/                 -> Histórico e evolução mensal
reports/                 -> Competências mensais
│
├── AAAA-MM/
│   ├── source/          -> PDFs originais exportados
│   ├── analysis/        -> Análises individuais (GA4 e GSC)
│   ├── output/          -> Resumo executivo da competência
│   └── manifest.json    -> Inventário e validação dos arquivos
...
- Cada competência possui uma pasta própria em `reports/`.
- `source/` contém apenas os PDFs originais.
- `analysis/` armazena apenas as análises individuais de Google Analytics 4 e Google Search Console.
- `output/` contém o resumo executivo final e a versão em Word.
- `manifest.json` registra os arquivos esperados e permite validar automaticamente se a competência está completa antes da análise.

---

## Estrutura Interna de Cada Competência

Cada competência mensal deverá ser totalmente autocontida.
...
- `source/`: preserva as evidências técnicas originais.
- `analysis/`: contém apenas as análises individuais de GA4 e Google Search Console.
- `output/`: contém exclusivamente os entregáveis finais da competência.
- `manifest.json`: valida se todos os arquivos obrigatórios da competência estão presentes antes do início da análise.


Não deverá existir mistura entre evidências, análises intermediárias e entregáveis finais.

## Convenção dos Arquivos

Os relatórios exportados do Google Analytics 4, Google Search Console e SEMrush deverão ser armazenados exatamente com os nomes gerados pelas ferramentas.

Não deverá haver renomeação manual dos arquivos.

A identificação de cada relatório será realizada automaticamente pelo processo de análise, utilizando padrões conhecidos dos nomes dos arquivos.

O arquivo `manifest.json` será responsável por registrar o mapeamento entre os nomes físicos dos PDFs e os tipos de relatório esperados.
...
1. Preparar a competência mensal seguindo o START_HERE.md.

2. O Codex deverá:

   ✓ validar a arquitetura;

   ✓ criar a competência (caso não exista);
   ...
   ✓ gerar ou validar o manifest.json.

3. O Codex deverá interromper a execução e solicitar:
...
4. Somente após receber "CONTINUAR", o Codex deverá:

   ✓ validar todos os PDFs;

   ✓ verificar se todos os relatórios obrigatórios estão presentes;

   ✓ interromper a execução caso exista qualquer inconsistência.

5. Após a validação, gerar:
...
6. Interromper novamente a execução e solicitar revisão humana.

7. Após aprovação do usuário:
...
A partir deste ponto o pipeline do Codex é considerado finalizado.

1. A geração dos documentos finais para entrega ao cliente (DOCX e PDF) será realizada em conjunto com o ChatGPT, utilizando como base o arquivo:

   reports/AAAA-MM/output/resumo_executivo.md

Após a revisão final, o ChatGPT produzirá:
...
- Relatórios técnicos em PDF preservados como evidência.
...
- Correlação entre todas as fontes de dados.
- Foco em tendências e impacto no projeto.
- PDFs preservados como evidência técnica.
- Consistência entre todas as entregas.
- Cada competência deve ser autocontida, preservando evidências, análises e entregáveis em sua própria estrutura.

---

## Evolução

A arquitetura foi projetada para crescer continuamente por meio da atualização da base de conhecimento, histórico do projeto, regras de análise e automações, mantendo estabilidade na metodologia de geração dos relatórios executivos.
# SFTL Reporting
